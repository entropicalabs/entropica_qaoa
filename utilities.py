...
def hamiltonian_from_hyperparams(reg: Iterable[int, QubitPlaceholder],
                                 singles: List[int, QubitPlaceholder],
                                 biases: List[float],
                                 pairs: List[int, QubitPlaceholder],
    ...
    ----------
    reg:
        The register to have the graph on.
    singles:
        The the qubits that have a bias term. Must be a subset of qubits
    biases:
        Values of the biases on the qubits specified in singles.
    pairs:
        The qubit pairs that have a non-zero coupling coefficient.
        Must be a subset of qubits x qubits
    couplings:
...
def random_hamiltonian(reg: List[int, QubitPlaceholder]) -> PauliSum:
    """
    ...
    ----------
    reg:
        The register to create the hamiltonian on.
    ...
    numb_biases = np.random.randint(len(reg))
    bias_qubits = random.sample(reg, numb_biases)
    ...
    for i in range(len(reg)):
        for j in range(i + 1, len(reg)):
            ...
            if are_coupled:
                ...
                hamiltonian.append(PauliTerm("Z", i, couple_coeff) *
                                   PauliTerm("Z", j))
...
def graph_from_hyperparams(reg: List[int, QubitPlaceholder],
                           singles: List[int, QubitPlaceholder],
                           biases: List[float],
                           pairs: List[int],

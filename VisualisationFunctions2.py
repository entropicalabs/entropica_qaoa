...
Terminology
~~~~~~~~~~~
You seem to use the term CostFn for hamiltonians whereas I use it for sth that
can be evaluated and/or passed to an optimizer. Not all cost_functions are
given as expectation values, think about minimizing the KL divergence between
the prob. distr. produced and a target distribution.
How about renaming your CostFn to CostHamiltonian.
...
def BuildQAOACircuit(n_qubits,QAOA_p,qubits,coefficients,beta_angles,gamma_angles):
    ...
    JL's comment
    ------------
    Yup, qaoa.cost_function.prepare_qaoa_ansatz(). Usage as follows (assuming imports)

    >>> hamiltonian = # some pauli sum
    >>> parameters = *QAOAParameters.from_hamiltonian(hamiltonian, timesteps)
    >>> # or any other way to prepare a subclass of AbstractQAOAParameters
    >>> program = prepare_qaoa_ansatz(parameters)
    """
...
def ApplyQAOACircuit(circuit):
    ...
    JL's comment
    ------------
    Actually I don't. Do we want to implement this, or leave it to the user
    since it is literally one line of code (two, if you need to create a
...
def BuildCostFn_Pyquil(qubits,coefficients):
    ...
    JL's comment
    ------------
    How do you want to use this? Where you used it in the Notebook I would
    replace that code later with
    >>> graph = make_random_graph(type="euclidean distances")
    >>> hamiltonian = hamiltonian_from_graph(graph)

    If we keep it, rename it to BuildHamiltonian, since (at least to me)
    cost_function is something I pass to an optimizer or evaluate.
    """
...
def EvaluateCostFunction(circuit,hamiltonian):
    ...
    JL's comment
    ------------
    Basically already done. Works as follows (assuming imports):

    >>> hamiltonian = # create an hamiltonian
    >>> circuit = # some parametric program
    >>> make_memory_map = # function that takes parameter values and returns a
    ..                    # memory map
    >>> cost_fn = PrepareAndMeasureOnWFSim(circuit,
    ..                                     make_memory_map,
    ..                                     hamiltonian,
    ..                                     WavefunctionSimulator(),
    ..                                     return_standard_deviation=False)
    >>> parameter = {"param_names_in_program": parameter_values}
    >>> exp_val = cost_fn(parameter_values)

    If the parameters are already hard compiled into the program, this would reduce to

    >>> cost_fn = PrepareAndMeasureOnWFSim(circuit, lambda x: None,
    ..                                     hamiltonian, WavefunctionSimulator(),
    ..                                     return_standard_deviation=False)
    >>> exp_val = cost_function([])
    """
...
def EvaluateCostFunctionVariance(circuit,hamiltonian):
    ...
    JL's comment
    ------------
    My cost_functions can be setup to automatically compute and return both
    the expectation value and its standard_deviation like so:

    >>> # important is the toggle return_standard_deviation=True
    >>> cost_fn = PrepareAndMeasureOnWFSim(circuit, lambda x: None,
    ..                                     hamiltonian, WavefunctionSimulator(),
    ..                                     return_standard_deviation=True)
    >>> exp_val, std_dev = cost_function([])
    """
...
def CostFunctionMatrix(n_qubits, Operators_Dict,coefficients):
    ...
    JL's comment
    ------------
    What is the use case of this. Can't it be replaced by pyquil.PauliSum.matrix()
    as implemented in my Pull request? If we consequently use pyquil.PauliSum's
    to represent the hamiltonian I see no problem.
    What is your Operators_Dict?
    """
...
def PlotBareCostFunction(n_qubits, Operators_Dict, coefficients):
    ...
    JL's comment
    ------------
    If we use pyquil.PauliSum for the hamiltonian, this would become
    >>> matrix = hamiltonian.matrix()
    >>> eigen_vals = matrix.diag()
    >>> plt.plot(eigen_vals) # plt.plot automaticall assumes x = [0,1,2...] if nothing is passed
    >>> plt.show()

    My implementation with the pretty labels |00> ... |11> is found in
    jl_code_dump.py plot_amplitudes()
    """

    # a little misleading, to call sth Matrix that is actually only a vector
...
def LandscapeParameters(n_qubits,qubits,coefficients,QAOA_p,betas2var,beta_range2var,betas,gammas2var,gamma_range2var,gammas):
    ...
    JL's comment
    ------------
    If using parametric circuits and a subclass of AbstractQAOAParameters
    for the circuits, it is probably easier to instantiate just one instance
    of the parameters and then update them from their raw representation.
    I.e. if you want to vary over the first two parameters, do:

    >>> ham = # hamiltonian
    >>> params = AlternatingOperatorsQAOAParameters.from_hamiltonian(ham, timesteps)
    >>> cost_fn = QAOACostFunctionOnWFSim(ham, params, WavefunctionSimulator())
    >>> params_raw = params.raw() # gives a flat list of all parameters
    >>> cost = np.empty((m, n))
    >>> for i, beta0 in enumerate(some_range):
    >>>     for j, beta1 in enumerate(some_other_range):
    >>>         params_raw[0], params_raw[1] = beta0, beta1 # update the raw parameters
    >>>         costs[i, j] = cost_fn(params_raw)

    Some more thinking would have to be put, into how one would go ahead to specify
    which of the betas and which of the gammas to vary and how that translates
    to which of the raw_parameters has to be varied.
    """
...
def PlotParametricCostFunction(params, param1, param2, param1_range, param2_range,param_labels,QAOA_p,qubits,coefficients,n_betas,hamiltonian,n_qubits):
    ...
    About varying one or two parameters: Don't do super long if ... else statements
    Instead write two functions _PlotParamtricCostFn1, _PlotParamtricCostFn2
    and make this
    >>> if param2:
    >>>     _PlotParamtricCostFn2(...)
    >>> else:
    >>>     _PlotParamtricCostFn1(...)
    """
    n_betas = n_qubits
...
def PlotParametricVariance(params, param1, param2, param1_range, param2_range,param_labels,QAOA_p,qubits,coefficients,n_betas,hamiltonian,n_qubits):
    ...
    JL's comment
    ------------
    As you say in the initial comments, let's merge this with PlotParametricCostFunction.
    Again it might come in handy, that my cost_functions can be configured to
    automatically calculate and return both, expectation value and standard deviation.
    """
...
def BuildPauliOps(n_qubits,qubits):
    ...
    JL's comment
    ------------
    let's hope rigetti accepts my Pull request, then there will be PauliSum.matrix()
    in a future version of pyquil and we can already start using it.
    """
